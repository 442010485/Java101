...
public class Assignment3 {
        ...
        static void max(String course, int[] ids, String[] studentsName, String[] courses, int[] grades) {
                ...
                for (int index = 0; index < ids.length; ++index) {
                        //if the courses at index equals to the course then print his course and grade
                        if (courses[index].equals(course) & moner1 < grades[index]  ) {
                                moner1 = grades[index];
                                moner2 = index;
                                System.out.format("[%d]:%s has the highest %d grade in %s\n", ids[moner2], studentsName[moner2], grades[moner2], course  );
                        }
                }
        }
        ...
        static void min(String course, int[] ids, String[] studentsName, String[] courses, int[] grades) {
                ...
                for (int index = 0; index < ids.length; ++index) {
                        //if the courses at index equals to the course then print his course and grade
                        if ( moner3 > grades[index] & courses[index].equals(course)) {
                                moner3 = grades[index];
                                moner4= index;
                                System.out.format("[%d]:%s has the lowest %d grade in %s\n", ids[moner4], studentsName[moner4], grades[moner4], course  );
                        }
                }
        }
        ...
        public static void main(String[] args) {
                ...
                max("math", ids, studentName, course, grade);


                //@todo calculate min grade in a course such as
                min("math", ids, studentName, course, grade);
